...
from dynamicoracles import ArcHybridDynamicOracle
from staticoracles import ArcHybridStaticOracle
...
def parse_corpus(corpus_fname, model_fname, feature_extractor, transition_system):
   ...
   parser = TransitionBasedParser(params, transition_system, feature_extractor)
...
def training_job(corpus_fname, k, p, seed, static, dev_fname, model_out_prefix):
   ...
   print "end"
   params.finalize()
   TRAIN_OUT_FILE = "%s-hybrid-k%s-p%s-seed%s" % (model_out_prefix, k, p, seed)
   if static:
   ...
   print "parsing"
   parsed = parse_corpus(dev_fname, TRAIN_OUT_FILE, feature_extractor, ArcHybridState)
   print "writing"
...
if __name__ == '__main__':
   ...
   for lang in "arabic basque catalan chinese czech english greek hungarian italian turkish".split():
      ...
      for seed in [1,2,3,4,5]:
         pool.apply_async(training_job, (corpus,0,0,seed,True,dev,model_prefix))
         for k in [1]:
            for p in [0.9,1]:
